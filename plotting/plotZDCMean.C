...
int plotZDCMean(char const *input = "/eos/cms/store/group/phys_heavyions/cmcginn/Run2026/HIPhysicsRawPrime10/404/187/") {
  ...
  const string outFileName = "output/plotZDCEmuHists_" + pdName + "_Run" + runNum + ".root";
  ...
  bool isMB = inputStr.find("HIPhysicsRawPrime");
  map<string, int> l1MB;
  ...
  bool isZeroBias = inputStr.find("PhysicsHIForward") != string::npos;
  ...
  bool isEmptyBunches = inputStr.find("HIExpress") != string::npos;
...
//cout << "Run: " <<  *runNumber << " lumi: " << *lumiSection << " event: " <<  *eventNumber << " sumZDCp: " << unpackerSumPlus << " sumZDCm: " << unpackerSumMinus << endl;
  ...
  for(auto const & trig : l1Map){
    ...
    for(auto const & extStr : extensions){
      c->SaveAs(Form("pdfDir/Online_ZDCEmu_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    trigTex->Draw("same");
    for(auto const & extStr : extensions){
      c12->SaveAs(Form("pdfDir/Online_ZDCEmuRatio_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    trigTex->Draw("same");
    for(auto const & extStr : extensions){
      cC->SaveAs(Form("pdfDir/Online_CorrAsymm_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    trigTex->Draw("same");
    for(auto const & extStr : extensions){
      cCj->SaveAs(Form("pdfDir/Online_CorrAsymmJet_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    cms->Draw("same");
    for(auto const & extStr : extensions){
      cA->SaveAs(Form("pdfDir/Online_Asymm_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    cms->Draw("same");
    for(auto const & extStr : extensions){
      cAVB->SaveAs(Form("pdfDir/Online_AsymmVBx_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cAVRB->SaveAs(Form("pdfDir/Online_AsymmVRelBxPlus_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cAVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cAVRB->SaveAs(Form("pdfDir/Online_AsymmVRelBxPlusZoom_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cAVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cAVRB->SaveAs(Form("pdfDir/Online_AsymmVRelBxMinus_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cAVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cAVRB->SaveAs(Form("pdfDir/Online_AsymmVRelBxMinusZoom_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cAVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cAVRB->SaveAs(Form("pdfDir/Online_AsymmVRelBxAbs_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cAVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cAVRB->SaveAs(Form("pdfDir/Online_AsymmVRelBxAbsZoom_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cAVRB;
    }
    ...
    cms->Draw("same");
    for(auto const & extStr : extensions){
      cAN->SaveAs(Form("pdfDir/Online_AsymmNorm_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    cms->Draw("same");
    for(auto const & extStr : extensions){
      cANVB->SaveAs(Form("pdfDir/Online_AsymmNormVBx_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cANVRB->SaveAs(Form("pdfDir/Online_AsymmNormVRelBxPlus_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cANVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cANVRB->SaveAs(Form("pdfDir/Online_AsymmNormVRelBxMinus_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cANVRB;
    }
    ...
    if(doBxMap){
      ...
      for(auto const & extStr : extensions){
	cANVRB->SaveAs(Form("pdfDir/Online_AsymmNormVRelBxMinusZoom_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cANVRB;
    }
    ...
    if(doBxMap){
      ...
      cms->Draw("same");
      for(auto const & extStr : extensions){
	cANVRB->SaveAs(Form("pdfDir/Online_AsymmNormVRelBxAbs_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cANVRB;
    }
    ...
    if(doBxMap){
      ...
      for(auto const & extStr : extensions){
	    cANVRB->SaveAs(Form("pdfDir/Online_AsymmNormVRelBxAbsZoom_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
      }
      delete cANVRB;
    }
    ...
    //lynn added code
    double ymin = hZDCM_withTrig[trig.first]->GetMinimum(1); // get min > 0 for log scale
    double ymax = hZDCM_withTrig[trig.first]->GetMaximum();

    double meanZDCM = hZDCM_withTrig[trig.first]->GetMean();
    cout << "Mean of ZDCM: " << meanZDCM << endl;
    double meanZDCP = hZDCP_withTrig[trig.first]->GetMean();
    cout << "Mean of ZDCP: " << meanZDCP << endl;
    double meanZDCM_fit = fit1nM->GetParameter(1);
    cout << "Mean of ZDCM fit: " << meanZDCM_fit << endl;
    double meanZDCP_fit = fit1n->GetParameter(1);
    cout << "Mean of ZDCP fit: " << meanZDCP_fit << endl;
    double diff_meanZDC = meanZDCP_fit-meanZDCM_fit;
    cout << "Difference of mean of fit:" <<  diff_meanZDC << endl;

    TLine* meanLineM = new TLine(meanZDCM, ymin, meanZDCM, ymax);
    meanLineM->SetLineColor(kBlue);
    meanLineM->SetLineWidth(2);
    meanLineM->SetLineStyle(2);
    meanLineM->Draw("same");

    TLine* meanLineP = new TLine(meanZDCP, ymin, meanZDCP, ymax);
    meanLineP->SetLineColor(kRed);
    meanLineP->SetLineWidth(2);
    meanLineP->SetLineStyle(2);
    meanLineP->Draw("same");

    leg2->AddEntry(hZDCP_withTrig[trig.first],"ZDC Plus ","l");
    leg2->AddEntry(hZDCM_withTrig[trig.first],"ZDC Minus","l");
    leg2->AddEntry(meanLineP,"ZDC+ mean","l");
    leg2->AddEntry(meanLineM,"ZDC- mean","l");
    ...
    fit1nM->Draw("same");
    for(auto const & extStr : extensions){
      c2->SaveAs(Form("pdfDir/Online_ZDCNeutronWithTrig_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
    ...
    leg4->Draw("same");
    for(auto const & extStr : extensions){
      c4->SaveAs(Form("pdfDir/TurnOn_%s_%s.%s", trig.first.c_str(), tag.c_str(), extStr.c_str()));
    }
  ...
  double mean = hZDCM_withTrig["L1_ZeroBias"]->GetMean();
  cout << "Mean of hZDCM_withTrig[L1_ZeroBias]: " << mean << endl;
