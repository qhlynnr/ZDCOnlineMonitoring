...
void plottingMacro(){
    ...
    cmsLabel = "#bf{CMS}";
    string cmsLabelPrelim = "Preliminary";
    string cmsLabel2 = "2026 PbPb (5.36 TeV)";
    ...
    cms->SetTextFont(42);
    cms->SetTextAlign(11); 
    TLatex* cmsprelim = new TLatex(0.178,0.92,cmsLabelPrelim.c_str());
    cmsprelim->SetNDC();
    ...
    TLatex* cmsLabel2Latex = new TLatex(0.630,0.92,cmsLabel2.c_str());
    cmsLabel2Latex->SetNDC();
    cmsLabel2Latex->SetTextSize(0.05);
    cmsLabel2Latex->SetTextFont(42);
    ...
    for (int i = 0; i < NTrig; i++){
     ...
     // leg2_LHCC->SetTextSize(0.04);
      ...
      //Experiment with fitting the total with a polynomial background
      //Since the 4 individual gaussian fits and the 4 altogether fits are separate
      //there is a small discrepancy between the two 

     /*total_plus->GetParameters(&parP[0]);
      total_minus->GetParameters(&parM[0]);

      fit1nM->SetParameters(parM[0],parM[1],parM[2]);
      fit1nM2->SetParameters(parM[3],parM[4],parM[5]);
      fit1nM3->SetParameters(parM[6],parM[7],parM[8]);
      fit1nM4->SetParameters(parM[9],parM[10],parM[11]); 
      fit1nP->SetParameters(parP[0],parP[1],parP[2]);
      fit1nP2->SetParameters(parP[3],parP[4],parP[5]);
      fit1nP3->SetParameters(parP[6],parP[7],parP[8]);
      fit1nP4->SetParameters(parP[9],parP[10],parP[11]); */
      ...
      hZDCM_L1_ZeroBias->GetYaxis()->SetTitle("Events");
      ...
      leg2_LHCC->AddEntry(hZDCP_L1_ZeroBias,"ZDC+ and Combined Fit ","l");
      leg2_LHCC->AddEntry(hZDCM_L1_ZeroBias,"ZDC- and Combined Fit ","l");

      leg2_LHCC->AddEntry(fit1nM,Form("ZDC- 1n: #mu=%.2f TeV, #sigma=%.2f TeV", muM1/1000, sigM1/1000), "l");
      leg2_LHCC->AddEntry(fit1nP,Form("ZDC+ 1n: #mu=%.2f TeV, #sigma=%.2f TeV", muP1/1000, sigP1/1000), "l");
      leg2_LHCC->AddEntry(fit1nM2,Form("ZDC- 2n: #mu=%.2f TeV, #sigma=%.2f TeV", muM2/1000, sigM2/1000), "l");
      leg2_LHCC->AddEntry(fit1nP2,Form("ZDC+ 2n: #mu=%.2f TeV, #sigma=%.2f TeV", muP2/1000, sigP2/1000), "l");
      leg2_LHCC->AddEntry(fit1nM3,Form("ZDC- 3n: #mu=%.2f TeV, #sigma=%.2f TeV", muM3/1000, sigM3/1000), "l");
      leg2_LHCC->AddEntry(fit1nP3,Form("ZDC+ 3n: #mu=%.2f TeV, #sigma=%.2f TeV", muP3/1000, sigP3/1000), "l");
      leg2_LHCC->AddEntry(fit1nM4,Form("ZDC- 4n: #mu=%.2f TeV, #sigma=%.2f TeV", muM4/1000, sigM4/1000), "l");
      leg2_LHCC->AddEntry(fit1nP4,Form("ZDC+ 4n: #mu=%.2f TeV, #sigma=%.2f TeV", muP4/1000, sigP4/1000), "l");

     // leg2_LHCC->AddEntry(total_minus,"","l");
    ...
    //  lumi->Draw("same");
     ...
     // lumisectionTex->Draw("same");
      ...
      hZDCP_L1_ZeroBias_Zoom->GetXaxis()->SetNdivisions(505);
      hZDCP_L1_ZeroBias_Zoom->GetYaxis()->SetNdivisions(504);
     ...
     // fit1nM4->Draw("same");
      ...
      TLatex* latex3n = new TLatex(0.29,0.64,"3n");
      latex3n->SetNDC();
      latex3n->SetTextSize(0.06);
      latex3n->SetTextFont(42);
      latex3n->SetTextColor(6);

      TLatex* latex4n = new TLatex(0.55,0.32,"4n");
      latex4n->SetNDC();
      latex4n->SetTextSize(0.06);
      latex4n->SetTextFont(42);
      latex4n->SetTextColor(6);

      TLatex* latex_onlyplus = new TLatex(0.45,0.83,"ZDC+ Only");
      latex_onlyplus->SetNDC();
      latex_onlyplus->SetTextSize(0.06);
      latex_onlyplus->SetTextFont(42);
      latex_onlyplus->SetTextColor(1);

      latex3n->Draw("same");
      latex4n->Draw("same");

      latex_onlyplus->Draw("same");
      latex->Draw("same");
      c2_LHCC->cd();
      leg2_LHCC->Draw("same");

      cmsLabel2Latex->Draw("same");
      cms->Draw("same");
      cmsprelim->Draw("same");
      ...
      // ------------------------------------------
        ...
        TCanvas* c_pull = new TCanvas("c_pull","c_pull",800,600);
        c_pull->cd();
        c_pull->SetTickx(1);
        c_pull->SetTicky(1);
        c_pull->SetTopMargin(0.09);
        c_pull->SetBottomMargin(0.11);
        c_pull->SetLeftMargin(0.13);
        c_pull->SetRightMargin(0.05); 
        TLegend* leg_pull = new TLegend(0.5,0.7,0.8,0.85);
        leg_pull->SetBorderSize(0);
        leg_pull->SetFillStyle(0);
        leg_pull->SetTextSize(0.03);  

        TH1D* hPullM = (TH1D*)hZDCM_L1_ZeroBias->Clone("hPullM");
        hPullM->SetLineColor(hexColorMinus);
        hPullM->SetLineWidth(2);
        hPullM->SetMarkerStyle(20);
        hPullM->GetYaxis()->SetRangeUser(-20, 20);

        TH1D* hPullP = (TH1D*)hZDCP_L1_ZeroBias->Clone("hPullP");
        hPullP->SetLineColor(hexColorPlus);
        hPullP->SetLineWidth(2);
        hPullP->SetMarkerStyle(20);
        hPullP->GetYaxis()->SetRangeUser(-20, 20);

        for (int i = 1; i < hZDCM_L1_ZeroBias->GetNbinsX(); i++) {
          double binValueM = hZDCM_L1_ZeroBias->GetBinContent(i);
          double fitValueM = total_minus->Eval(hZDCM_L1_ZeroBias->GetBinCenter(i));
          double errorM = hZDCM_L1_ZeroBias->GetBinError(i);
          double pullM = (binValueM - fitValueM) / errorM;
          hPullM->SetBinContent(i, pullM);
          hPullM->SetBinError(i, 0); // Set error to 0 for pull plot
        }

        for (int i = 1; i < hZDCP_L1_ZeroBias->GetNbinsX(); i++) {
          double binValueP = hZDCP_L1_ZeroBias->GetBinContent(i);
          double fitValueP = total_plus->Eval(hZDCP_L1_ZeroBias->GetBinCenter(i));
          double errorP = hZDCP_L1_ZeroBias->GetBinError(i);
          double pullP = (binValueP - fitValueP) / errorP;
          hPullP->SetBinContent(i, pullP);
          hPullP->SetBinError(i, 0); // Set error to 0 for pull plot
        }

        hPullM->Draw();
        hPullP->Draw("same");
        leg_pull->AddEntry(hPullM,"ZDC- Pull","l");
        leg_pull->AddEntry(hPullP,"ZDC+ Pull","l");
        leg_pull->Draw("same");
        cms->Draw("same");
        ...
        trigTex->Draw("same");
        for(auto const & extStr : extensions){
          c_pull->SaveAs(Form("%s/PullPlot_%s_%s.%s",outDirPlot.c_str(),triglabel.c_str(), tag.c_str(), extStr.c_str()));
        }
       // ------------------------------------------
      /* Internal Check*/
      // ------------------------------------------
